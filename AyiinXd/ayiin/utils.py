    ...
    try:
        r = await bot(
            CreateChannelRequest(
                title="NANDEE-Userbot Logs",
                about="» Group log Created by: NANDEE-Userbot\n\n» Support : @suportsippuserbot\n» Support : @alimbanget_2",
                megagroup=True,
            ),
        )
...
async def autobot():
    ...
    if who.username:
        ...
        username = f"NANDEE{(str(who.id))[5:]}bot"
    bf = "@BotFather"
    ...
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = f"NANDEE{(str(who.id))[6:]}{str(ran)}bot"
        await bot.send_message(bf, username)
        await asyncio.sleep(1)
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "AyiinXd/resources/MG-20220810-WA0019.jpg")
            await asyncio.sleep(3)
